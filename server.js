...
async function runBasketball(emit) {
  ...
  const { picks, combiPool, mkP, dropReasons, dropSigCounts } = buildPickFactory(1.60, calib.epBuckets || {}, 'basketball', { onCandidate: _basketballOnCandidate });
  ...
  const _dropFmtNba = formatDropReasons(dropReasons, dropSigCounts);
...
async function runHockey(emit) {
  ...
  const { picks, combiPool, mkP, dropReasons, dropSigCounts } = buildPickFactory(1.60, calib.epBuckets || {}, 'hockey', { onCandidate: _hockeyOnCandidate });
  const MIN_EDGE = 0.055;
  let totalEvents = 0;
  let apiCallsUsed = 0;
  ...
  const _dropFmtNhl = formatDropReasons(dropReasons, dropSigCounts);
...
async function runBaseball(emit) {
  ...
  const { picks, combiPool, mkP, dropReasons, dropSigCounts } = buildPickFactory(1.60, calib.epBuckets || {}, 'baseball', { onCandidate: _baseballOnCandidate });
  const MIN_EDGE = 0.055;
  let totalEvents = 0;
  let apiCallsUsed = 0;
  ...
  // v12.2.3: drop-reasons telemetrie.
  const _dropFmtMlb = formatDropReasons(dropReasons, dropSigCounts);
...
async function runFootballUS(emit) {
  ...
  const { picks, combiPool, mkP, dropReasons, dropSigCounts } = buildPickFactory(1.60, calib.epBuckets || {}, 'american-football', { onCandidate: _nflOnCandidate });
  const MIN_EDGE = 0.055;
  let totalEvents = 0;
  let apiCallsUsed = 0;
  ...
  // v12.2.3: drop-reasons telemetrie.
  const _dropFmtNfl = formatDropReasons(dropReasons, dropSigCounts);
...
async function runHandball(emit) {
  ...
  const { picks, combiPool, mkP, dropReasons, dropSigCounts } = buildPickFactory(1.60, calib.epBuckets || {}, 'handball', { onCandidate: _handballOnCandidate });
  const MIN_EDGE = 0.055;
  let totalEvents = 0;
  let apiCallsUsed = 0;
  ...
  // v12.2.3: drop-reasons telemetrie.
  const _dropFmtHb = formatDropReasons(dropReasons, dropSigCounts);
...
async function runPrematch(emit) {
  ...
  // v12.5.9: scraper-health fire-and-forget (was v12.5.8 await — block tot 7s
  // op trage scrapers, kon SSE-stream stalle). Emit pas wanneer check klaar
  // is, met 4s timeout fallback. Scan-flow wacht niet meer op health-check.
  if (OPERATOR.scraping_enabled) {
    (async () => {
      try {
        ...
        emit({ log: `🔌 Scraper-health: ${e?.message || 'unknown'}` });
      }
    })();
  }
...
app.listen(PORT, () => {
  ...
  // v12.5.7: master-default fallback als scraper_sources empty.
  // v12.5.10: helper geëxtract zodat we 'm NA `loadOperatorState()` kunnen
  ...
  // v12.5.10: scraper-restore moet OOK na loadOperatorState() draaien,
  // anders is OPERATOR.scraping_enabled nog default-false → master-default
  // fallback werkt niet. Pre-v12.5.10 had dit als sync-block hierboven
  // staan (race-conditie: synced vóór async load).
  loadOperatorState()
    .then(() => { restoreScraperSourcesFromCalib(); })
