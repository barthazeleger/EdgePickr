 ...
 * @param {object} args { fixtureId, markt, preferredBookie }
...
async function fetchSnapshotClosing(supabase, args = {}) {
  ...
  const { fixtureId, markt, preferredBookie = '' } = args;
  ...
  try {
    ...
    if (lowerPref) {
      const preferredRow = data.find(r => String(r.bookmaker || '').toLowerCase() === lowerPref);
