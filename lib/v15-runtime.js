...
function classifyTwoWaySelection(quote, homeName, awayName, opts = {}) {
  const selection = String(quote?.selection || '').trim();
  if (!selection || !homeName || !awayName) return null;
  const minScore = Number.isFinite(opts.minScore) ? opts.minScore : 60;
  const homeScore = teamMatchScore(selection, homeName);
  const awayScore = teamMatchScore(selection, awayName);
  if (homeScore >= minScore && homeScore > awayScore) return 'home';
  if (awayScore >= minScore && awayScore > homeScore) return 'away';
  const exact = selection.toLowerCase();
  if (exact === 'home' || exact === 'home team') return 'home';
  if (exact === 'away' || exact === 'away team') return 'away';
  return null;
}

function selectTwoWaySharpQuote(sample, homeName, awayName) {
  const candidates = (Array.isArray(sample) ? sample : [])
    .map(q => ({ quote: q, selection: classifyTwoWaySelection(q, homeName, awayName) }))
    .filter(x => x.selection && x.quote?.bookie && Number.isFinite(Number(x.quote.price)) && Number(x.quote.price) > 1);
  if (!candidates.length) return null;
  return candidates.find(x => /pinnacle|betfair/i.test(String(x.quote.bookie || ''))) || candidates[0];
}

function preferredCoverageFromBookies(bookies = [], preferredBookies = [], minCount = 3) {
  const seen = [...new Set((Array.isArray(bookies) ? bookies : [])
    .map(b => String(b || '').toLowerCase().trim())
    .filter(Boolean))];
  const preferred = (Array.isArray(preferredBookies) && preferredBookies.length
    ? preferredBookies
  ...
  for (const pref of preferred) {
    if (seen.some(b => b === pref || b.includes(pref) || pref.includes(b))) matched.push(pref);
  }
  ...
  return {
    ...
    rate: preferred.length ? +(count / preferred.length * 100).toFixed(1) : 0,
    bookies: matched,
    pass: count >= minCount,
  };
}
