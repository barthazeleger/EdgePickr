...
async function fetchNhlGoaliePreview(gameId) {
  ...
  try {
    // v10.10.22 fase 2: safeFetch i.p.v. raw fetch (SSRF consistency hardening)
    const { safeFetch } = require('./scraper-base');
    const resp = await safeFetch(`https://api-web.nhle.com/v1/gamecenter/${encodeURIComponent(gameId)}/landing`, {
    ...
    const value = extractNhlGoaliePreview(json);
