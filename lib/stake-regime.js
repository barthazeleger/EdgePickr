 ...
 *   - `early_caution`  — 50-99 settled zonder bewezen positieve CLV → kelly 0.35, unit ×1.0
...
function evaluateStakeRegime(input = {}) {
  ...
  // 50-99 settled is geen "startfase" meer, maar ook nog geen proven-edge.
  // Houd dezelfde sizing als exploratory, maar label eerlijker zodat een
  // operator bij 59 settled bets niet denkt dat de counter fout is.
  addReason('early_caution', `50-99 settled zonder bewezen step-up bewijs (settled ${totalSettled}, CLV ${longTermClvPct ?? 'n/a'})`);
  return { regime: 'early_caution', kellyFraction: 0.35, unitMultiplier: 1.0, reasons };
}
