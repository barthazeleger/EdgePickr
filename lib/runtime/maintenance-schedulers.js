...
const { runCoverageAudit, postCoverageInsight } = require('../jobs/coverage-audit');
...
module.exports = function createMaintenanceSchedulers(deps) {
  const {
    ...
    getUnitEur,
    // v15.4 optional deps. coverage-audit + slice-readiness fallen schoon weg
    // als caller (test.js) ze niet meegeeft → schedulers loggen + return.
    getLeagues = null,        // fn () → { football: [...], basketball: [...], ... }
    getAppReleaseDate = null, // fn () → 'YYYY-MM-DD' (lib/app-meta.APP_RELEASE_DATE)
  } = deps;
  ...
  // v15.4 · Coverage-audit (PLAN §5.1). Dagelijks tellen welke geconfigureerde
  // ligas in laatste 90d picks produceerden, dormant lijst naar operator-inbox
  // als read-only `coverage_insight`. Dedup via calib.coverage_audit_last_run
  // — niet vaker dan 1× per 24u zelfs als scheduler-tick herhaalt.
  let _lastCoverageAuditAtMs = 0;
  let _lastCoverageAuditResult = null;
  async function runCoverageAuditJob() {
    if (typeof getLeagues !== 'function') {
      console.warn('🗺️ Coverage-audit: geen getLeagues dep injected, skip');
      return null;
    }
    const COOLDOWN_MS = 24 * 60 * 60 * 1000;
    if (Date.now() - _lastCoverageAuditAtMs < COOLDOWN_MS) return _lastCoverageAuditResult;
    let leagues;
    try { leagues = getLeagues() || {}; } catch (e) { console.warn('Coverage-audit getLeagues fout:', e?.message || e); return null; }
    if (!leagues || Object.keys(leagues).length === 0) {
      console.warn('🗺️ Coverage-audit: lege leagues map, skip');
      return null;
    }
    try {
      const result = await runCoverageAudit({ supabase, leagues, daysWindow: 90 });
      _lastCoverageAuditAtMs = Date.now();
      _lastCoverageAuditResult = result;
      console.log(`🗺️ Coverage-audit: ${result.leaguesActive}/${result.leaguesTotal} actief · ${result.dormantLeagues.length} dormant (${result.fixturesSeen} fixtures)`);
      // Eén notificatie per dag. Bestaande dedup via cooldown-flag boven.
      await postCoverageInsight({ supabase, result });
      return result;
    } catch (e) {
      console.warn('Coverage-audit fout:', e?.message || e);
      return null;
    }
  }
  function scheduleCoverageAudit() {
    const INTERVAL_MS = 24 * 60 * 60 * 1000;
    const FIRST_RUN_MS = 30 * 60 * 1000; // +30min boot — pas nadat init-jobs runnen
    setTimeout(() => {
      runCoverageAuditJob().catch(() => {});
      setInterval(() => runCoverageAuditJob().catch(() => {}), INTERVAL_MS);
    }, FIRST_RUN_MS);
    console.log('🗺️ Coverage-audit scheduler actief (dagelijks vanaf +30min boot)');
  }
  function getCoverageAuditStatus() {
    return { lastRunAtMs: _lastCoverageAuditAtMs || null, lastResult: _lastCoverageAuditResult };
  }

  // v15.4 · Slice-readiness scheduler voor v15.5 (PLAN §5.1, §6 operator_action).
  // Zodra v15.4 ≥7d in productie staat én geen kritische scan-errors in laatste
  // 24u → post operator_action notif: "v15.5 ready to ship · trigger /ship v15.5".
  // Dedup persistent via calib.slice_readiness_v15_5_notified zodat operator de
  // melding maar één keer krijgt.
  async function runSliceReadinessCheckV155() {
    if (typeof getAppReleaseDate !== 'function') return null;
    const releaseStr = String(getAppReleaseDate() || '').trim();
    const ms = Date.parse(releaseStr);
    if (!Number.isFinite(ms)) return null;
    // Pas vanaf v15.4-release-datum tellen. We verifiëren ook dat we boven
    // 15.4.x draaien — anders is het release-datum niet die van v15.4.
    let appVer = null;
    try { appVer = require('../app-meta').APP_VERSION; } catch (_) { appVer = null; }
    if (typeof appVer !== 'string' || !/^15\.4(\.|$)/.test(appVer)) return null;
    const elapsedMs = Date.now() - ms;
    if (elapsedMs < 7 * 86400000) return { ready: false, reason: 'soak_window', elapsedMs };

    // Dedup: skip als calib al genoteerd heeft dat we hebben gemeld.
    let calibState;
    try { calibState = (typeof loadCalib === 'function' ? loadCalib() : {}) || {}; }
    catch (_) { calibState = {}; }
    if (calibState.slice_readiness_v15_5_notified) return { ready: false, reason: 'already_notified' };

    // "Geen kritische errors" proxy: geen kill_switch of heartbeat_miss
    // notificaties in laatste 24u. Bij twijfel → niet melden (operator krijgt
    // dan gewoon volgende dag een nieuwe kans).
    try {
      const sinceIso = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
      const { data: critical, error } = await supabase
        .from('notifications')
        .select('id, type, created_at')
        .in('type', ['kill_switch', 'heartbeat_miss'])
        .gte('created_at', sinceIso)
        .limit(5);
      if (error) {
        console.warn('Slice-readiness check: notifications read fout', error.message);
        return null;
      }
      if (Array.isArray(critical) && critical.length > 0) {
        return { ready: false, reason: 'critical_alert_present', count: critical.length };
      }
    } catch (e) {
      console.warn('Slice-readiness check fout:', e?.message || e);
      return null;
    }

    const sentResult = await sendOperatorNotification({
      supabase,
      category: 'operator_action',
      type: 'slice_readiness',
      title: '🚀 v15.5 ready to ship',
      body: `v15.4 staat ${Math.floor(elapsedMs / 86400000)}d in productie zonder kritische alerts. Trigger \`/ship v15.5 — CLV-meting overhaul\` om de volgende slice te starten.`,
      sendPush: typeof notify === 'function'
        ? (cat, title, body) => notify(`${title}\n${body}`, 'slice_readiness')
        : null,
    });
    if (sentResult.ok) {
      try {
        calibState.slice_readiness_v15_5_notified = new Date().toISOString();
        if (typeof saveCalib === 'function') await saveCalib(calibState);
      } catch (e) { console.warn('Slice-readiness dedup persist fout:', e?.message || e); }
      console.log('🚀 v15.5 slice-readiness: operator-inbox notificatie geland.');
    }
    return { ready: true, sent: sentResult.ok, elapsedMs };
  }
  function scheduleSliceReadiness() {
    const INTERVAL_MS = 24 * 60 * 60 * 1000;
    const FIRST_RUN_MS = 45 * 60 * 1000; // +45min boot (na coverage-audit)
    setTimeout(() => {
      runSliceReadinessCheckV155().catch(() => {});
      setInterval(() => runSliceReadinessCheckV155().catch(() => {}), INTERVAL_MS);
    }, FIRST_RUN_MS);
    console.log('🚀 Slice-readiness scheduler actief (dagelijks vanaf +45min boot, target v15.5)');
  }
  ...
  return {
    ...
    runCoverageAuditJob,
    getCoverageAuditStatus,
    runSliceReadinessCheckV155,
  };
};
