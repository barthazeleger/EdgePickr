...
module.exports = function createOpenBetsChecker(deps) {
  ...
  return async function checkOpenBetResults(userId = null) {
    ...
    for (const bet of openBets) {
      ...
      if (uitkomst) {
        // v11.3.23 F1 (Codex #1): gebruik bet.userId als owner-scope wanneer
        // beschikbaar. Bij globale cron-run (userId=null) voorkomt dit dat
        // settled bets zonder user-scope worden geschreven én dat push naar
        // "alle users" (null) gaat ipv de daadwerkelijke bet-owner.
        const ownerId = bet.userId || userId || null;
        await updateBetOutcome(bet.id, uitkomst, ownerId);
