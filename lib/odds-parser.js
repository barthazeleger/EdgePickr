...
function parseGameOdds(oddsResp, homeTeam, awayTeam) {
  ...
  for (const bk of bookmakers) {
    ...
    for (const bet of (bk.bets || [])) {
      ...
      const isExcludedDerivative = betName.includes('team total') || betName.includes('corner') ||
        betName.includes('card') || betName.includes('booking') || betName.includes('player');
      const isFullGameTotalByName = !isHalfOrF5Bet && !isExcludedDerivative && (
        betName === 'over/under' ||
        betName.includes('goals over/under') ||
        betName.includes('runs over/under') ||
        betName.includes('total runs') ||
        betName.includes('total goals') ||
        betName.includes('total points') ||
        betName.includes('game total')
      );
      const isFullGameSpreadByName = !isHalfOrF5Bet && !isExcludedDerivative && !betName.includes('draw no bet') && (
        betName.includes('asian handicap') ||
        betName === 'handicap' ||
        betName === 'spread' ||
        betName.includes('run line') ||
        betName.includes('puck line')
      );
      if ((betId === 2 || betId === 3 || isFullGameTotalByName || isFullGameSpreadByName) && !isHalfOrF5Bet) {
      ...
      if (isTeamTotalBet) {
        ...
        // v12.1.12: NL labels toegevoegd (Unibet.nl/Toto.nl/BetCity.nl gebruiken
        // 'reguliere speeltijd' / 'inclusief verlenging'). Zonder deze detectie
        // viel alles terug op 'unknown' + bookie-blacklist, waardoor Unibet's
        // incl-OT TT (1.97) per ongeluk werd geskipt ook al was de markt expliciet
        // gelabeld.
        const ttScope = (
          betName.includes('regulation') ||
          betName.includes('regular time') ||
          betName.includes('60 min') ||
          betName.includes('reguliere speeltijd') ||
          betName.includes('excl. ot') ||
          betName.includes('excl ot') ||
          betName.includes('excluding overtime')
        )
          ? 'regulation'
          : (
              (betName.includes('incl') && betName.includes('ot')) ||
              betName.includes('overtime included') ||
              betName.includes('including overtime') ||
              betName.includes('inclusief verlenging') ||
              betName.includes('inclusief ot')
            )
            ? 'incl_ot'
            : 'unknown';
        for (const v of (bet.values || [])) {
  ...
  //
  // v15.0.11 (Bug B definitive fix): voor PAIRED O/U + spreads markets is de
  // v12.0.0-aanname onjuist gebleken. Bookies (vooral MLB via api-football)
  // bieden routinematig alt-lines onder dezelfde betId/betName als main-line.
  // Bij Over 9.5 met main 1.85 + alt 3.30 picks dedupeBestPrice de alt → over
  // 9.5 én under 9.5 krijgen beide ~3.30 → overround sum=0.61 → divergence-
  // gate blokkeert. Voor totals + spreads gebruiken we daarom dedupeMainLine
  // (lowest-price per bookie+point+side = main-line by construction). Voor
  // andere markten (ml/btts/3way/dnb/dc) blijft best-price-dedupe correct.
  ...
  return {
    ...
    // v15.0.11 Bug B definitive: paired O/U + spreads → main-line dedupe
    // (lowest price per bookie+point+side). Voorkomt alt-line leak die
    // overround-gate corrupteert (zie Athletics vs Royals MLB total 9.5
    // tot=0.606 = beide kanten op alt-line 3.30).
...
function convertAfOdds(afBookmakers, hm, aw) {
  return afBookmakers.map(bk => {
    ...
    const isMainTotalBet = (b) => {
      const name = String(b?.name || '').toLowerCase();
      return b?.id === 5 || (
        !name.includes('half') && !name.includes('period') && !name.includes('quarter') &&
        !name.includes('inning') && !name.includes('team total') && !name.includes('corner') &&
        !name.includes('card') && !name.includes('booking') && !name.includes('player') &&
        (name === 'over/under' || name.includes('goals over/under') || name.includes('runs over/under') ||
          name.includes('total runs') || name.includes('total goals') || name.includes('total points') ||
          name.includes('game total'))
      );
    };
    const ou = bk.bets?.find(isMainTotalBet);
    if (ou) {
      markets.push({
        key: 'totals',
    ...
    const isMainSpreadBet = (b) => {
      const name = String(b?.name || '').toLowerCase();
      return !name.includes('draw no bet') &&
        !name.includes('half') && !name.includes('period') && !name.includes('quarter') &&
        !name.includes('inning') && !name.includes('corner') && !name.includes('card') &&
        (b?.id === 12 || name.includes('asian handicap') || name === 'handicap' ||
          name === 'spread' || name.includes('run line') || name.includes('puck line'));
    };
    const ah = bk.bets?.find(isMainSpreadBet);
    if (ah) {
      markets.push({
        key: 'spreads',
    ...
    // v12.5.13: Double Chance parsing. api-sports.io's bet-id voor DC varieert
    // (soms 12, soms 14, soms apart bet-name) — zoek op naam-string i.p.v.
    // bet-id voor robustheid. Server.js DC-block (regel ~6700) verwachtte een
    // market met `key='double_chance'` op de convertAfOdds-output, maar
    // pre-v12.5.13 produceerde convertAfOdds die key nooit → DC-block kreeg
    // altijd 0 bookies → `dc_no_market = 100%` van pre-mkP funnel.
    // values van DC api-sports: { value: '1X' | '12' | 'X2' | 'Home/Draw' | ... }
    const dc = bk.bets?.find(b => (b.name || '').toLowerCase().includes('double chance'));
    if (dc) {
      markets.push({
        key: 'double_chance',
        outcomes: dc.values.map(v => {
          const raw = (v.value || '').toLowerCase().replace(/\s+/g, '');
          // Map naar canonical 1x/12/x2 (zelfde shape als
          // server.js DC-block + lib/snapshots flattenFootballBookies verwacht).
          let key = null;
          if (raw === '1x' || raw === 'home/draw' || raw === 'homeordraw') key = '1x';
          else if (raw === '12' || raw === 'home/away' || raw === 'homeoraway') key = '12';
          else if (raw === 'x2' || raw === 'draw/away' || raw === 'draworaway') key = 'x2';
          if (!key) return null;
          const price = parseFloat(v.odd) || 0;
