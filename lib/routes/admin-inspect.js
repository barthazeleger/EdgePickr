...
module.exports = function createAdminInspectRouter(deps) {
  ...
  // ── GET /api/admin/v2/expansion-graduation-candidates (v15.3.0) ──────────
  // Aggregeert settled expansion-shadow rows (rejected_reason='expansion_shadow_paper',
  // shadow=true, result IN (W,L,P)) per liga uit laatste N weken (default 8).
  // Past 6 multi-dim promotion-gates toe via lib/graduation-evaluator.
  // Anti-BTTS-bug: alle 6 gates moeten passeren, geen single-metric promotion.
  router.get('/admin/v2/expansion-graduation-candidates', requireAdmin, async (req, res) => {
    try {
      const weeks = Math.max(2, Math.min(24, parseInt(req.query.weeks, 10) || 8));
      const recentWeeks = Math.max(1, Math.min(weeks, parseInt(req.query.recent_weeks, 10) || 4));
      const sinceMs = Date.now() - weeks * 7 * 86400000;
      const recentSinceMs = Date.now() - recentWeeks * 7 * 86400000;

      const { data: rows, error } = await supabase
        .from('pick_candidates')
        .select('id, fixture_id, sport, market_type, markt_label, bookmaker, bookmaker_odds, kickoff_ms, result, clv_pct, settled_at, source_attribution, sharp_anchor, playability, model_run_id')
        ...
        .limit(20000);
      if (error) {
        console.error('[expansion-graduation]', error.message);
        return res.status(500).json({ error: 'Interne fout · check server logs' });
      }
      ...
      return res.json({
        windowWeeks: weeks,
        recentWindowWeeks: recentWeeks,
        gates: result.gates,
        preferredBookies: result.preferredBookies,
        totalSettled: result.summary.totalRows,
        leagueCount: result.summary.leagueCount,
        graduationReadyCount: result.summary.graduationReadyCount,
        candidates: result.candidates,
        note: 'Multi-dimensional gates voorkomen single-metric promotion (BTTS-bug doctrine). All 6 gates moeten passeren voordat een liga graduation_ready=true krijgt. Auto-promote naar AF_FOOTBALL_LEAGUES vereist handmatige api-sports league.id resolve; daarom genereert de scheduler alleen een notification, geen automatische config-write.',
      });
    } catch (e) {
      console.error('[expansion-graduation]', e?.message || e);
      res.status(500).json({ error: 'Interne fout · check server logs' });
    }
  });
