  ...
  router.get('/admin/v2/expansion-graduation-candidates', requireAdmin, async (req, res) => {
    try {
      ...
      const { data: rows, error } = await supabase
        .from('pick_candidates')
        .select('id, fixture_id, sport, market_type, markt_label, bookmaker, bookmaker_odds, kickoff_ms, result, clv_pct, settled_at, source_attribution, sharp_anchor, model_run_id')
