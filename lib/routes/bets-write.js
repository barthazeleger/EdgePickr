...
// v12.2.4 (F1): per-key in-memory cache voor de fixture-resolver fallback.
// Voorkomt dat herhaalde lookups voor dezelfde (sport, datum, wedstrijd)
// telkens een Supabase-query genereren. TTL 30 min (fixtures muteren niet
// binnen die window). Ook negative results worden gecacht (resolved=null)
// om DoS-vector via bulk-bet-creates met onbekende teams te dichten.
// v12.2.16 (R7): inflight-promise dedup — N concurrent calls op dezelfde
// koude key triggeren maar 1 Supabase query. Voorheen was er een race
// gap: alle N calls zagen `cached===undefined` vóór de eerste de cache vulde.
const _fixtureResolverCache = new Map();
const _fixtureResolverInflight = new Map();
const FIXTURE_RESOLVER_TTL_MS = 30 * 60 * 1000;
const FIXTURE_RESOLVER_CACHE_MAX = 1000;

function _cacheKey(bet) {
  return `${normalizeSportKey(bet.sport)}|${bet.datum}|${String(bet.wedstrijd || '').toLowerCase().trim()}`;
}
function _cacheGet(key) {
  const e = _fixtureResolverCache.get(key);
  if (!e) return undefined;
  if (Date.now() - e.at > FIXTURE_RESOLVER_TTL_MS) {
    _fixtureResolverCache.delete(key);
    return undefined;
  }
  return e.value;
}
function _cacheSet(key, value) {
  if (_fixtureResolverCache.size >= FIXTURE_RESOLVER_CACHE_MAX) {
    // Evict oudste entry (Map preserveert insertion order).
    const firstKey = _fixtureResolverCache.keys().next().value;
    if (firstKey) _fixtureResolverCache.delete(firstKey);
  }
  _fixtureResolverCache.set(key, { value, at: Date.now() });
}

async function resolveFixtureIdForBet(supabase, bet) {
  try {
    if (!bet?.datum || !bet?.wedstrijd || !bet?.sport) return null;
    // Cache eerst — bespaart Supabase-quota bij herhaalde lookups.
    const cacheKey = _cacheKey(bet);
    const cached = _cacheGet(cacheKey);
    if (cached !== undefined) return cached;
    // v12.2.16 (R7): inflight-promise dedup — als er al een lookup voor deze
    // key onderweg is, wacht op die ipv een tweede Supabase query starten.
    const inflight = _fixtureResolverInflight.get(cacheKey);
    if (inflight) return inflight;
    const promise = (async () => {
      try {
        return await _resolveFixtureUncached(supabase, bet, cacheKey);
      } finally {
        _fixtureResolverInflight.delete(cacheKey);
      }
    })();
    _fixtureResolverInflight.set(cacheKey, promise);
    return promise;
  } catch (_) {
    return null;
  }
...
async function _resolveFixtureUncached(supabase, bet, cacheKey) {
  try {
    ...
    if (hits.length === 0) {
      ...
      if (swapped.length === 1) {
        _cacheSet(cacheKey, swapped[0].id);
        return swapped[0].id;
      }
    }
    _cacheSet(cacheKey, null);
...
function _resetFixtureResolverCacheForTests() {
  _fixtureResolverCache.clear();
  _fixtureResolverInflight.clear();
}
 ...
 *   - DELETE /api/bets/:id/clv        — handmatig CLV/closing odds wissen voor één bet
...
module.exports = function createBetsWriteRouter(deps) {
  ...
  router.delete('/bets/:id/clv', async (req, res) => {
    try {
      const userId = req.user?.id;
      if (rateLimit('betwrite:' + userId, 60, 60 * 1000)) {
        return res.status(429).json({ error: 'Te veel bet-writes · wacht een minuut' });
      }
      const id = parseInt(req.params.id, 10);
      if (isNaN(id) || id <= 0) return res.status(400).json({ error: 'Ongeldig ID' });
      let updateQuery = supabase.from('bets').update({
        clv_odds: null,
        clv_pct: null,
        sharp_clv_odds: null,
        sharp_clv_pct: null,
  ...
  router.get('/bets/:id/current-odds', async (req, res) => {
    try {
      ...
      // v12.2.4 (F1): aparte rate-limit op de fallback-resolver om DoS-vector
      // op Supabase quota te dichten (10 fallback-lookups/min/user). Cache
      // binnen resolveFixtureIdForBet doet 90% van de werklast voor herhalingen.
      if (!bet.fixture_id) {
        if (rateLimit('fixtureresolver:' + userId, 10, 60 * 1000)) {
          return res.status(429).json({ error: 'Te veel fallback-lookups · wacht een minuut' });
        }
      ...
      // v12.2.5 (F7): bookie-specifieke lookup. Prio: (1) exact match op bet.tip,
      // (2) andere preferred bookies, (3) hele markt. Drift werd voorheen
      // misleidend berekend tegen highest preferred (bv. Unibet @ 1.97 vs
      // Bet365 @ 1.86 → -5.58% drift terwijl Bart bij Unibet zat).
      const tipLc = String(bet.tip || '').toLowerCase().trim();
      const tipBookie = tipLc ? bookmakers.find(b => String(b.name || '').toLowerCase() === tipLc
                                                || String(b.name || '').toLowerCase().includes(tipLc)
                                                || tipLc.includes(String(b.name || '').toLowerCase())) : null;
      const findBest = (pool) => {
        let b = { price: 0, bookie: null };
        for (const bk of pool) {
          const price = resolveOddFromBookie(bk, bet.markt || '', { matchName: bet.wedstrijd || '', sport });
          if (Number.isFinite(price) && price > b.price) {
            b = { price: +price.toFixed(3), bookie: bk.name };
          }
        }
        return b;
      };

      // Probeer eerst exact bet.tip → geeft eerlijke drift-vergelijking
      let best = { price: 0, bookie: null };
      let priceSource = 'none';
      if (tipBookie) {
        best = findBest([tipBookie]);
        if (best.price > 0) priceSource = 'logged_bookie';
      }
      // Fallback 1: andere preferred bookies (best-of)
      if (best.price <= 0 && preferredBks.length) {
        best = findBest(preferredBks);
        if (best.price > 0) priceSource = 'other_preferred';
      }
      // Fallback 2: hele markt
      if (best.price <= 0) {
        best = findBest(bookmakers);
        if (best.price > 0) priceSource = 'market_best';
      }
      ...
      res.json({
        ...
        priceSource, // 'logged_bookie' | 'other_preferred' | 'market_best'
        deltaAbs,
        deltaPct,
        direction,
        impliedLogged,
...
module.exports._resetFixtureResolverCacheForTests = _resetFixtureResolverCacheForTests;
