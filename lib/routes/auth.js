...
module.exports = function createAuthRouter(deps) {
  ...
  router.post('/auth/verify-code', async (req, res) => {
    try {
      ...
      // v12.2.9 (F6): probeer async-lookup (ondersteunt persistent store na
      // server-restart). Backwards-compat: als loginCodes alleen sync .get
      // ondersteunt, valt terug op die.
      const entry = typeof loginCodes.getAsync === 'function'
        ? await loginCodes.getAsync(emailKey)
        : loginCodes.get(emailKey);
