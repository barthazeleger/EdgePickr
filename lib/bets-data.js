...
module.exports = function createBetsData(deps) {
  ...
  // v11.3.23 C3 (Codex #2): atomic bet_id allocatie. MAX(bet_id)+1 is niet
  // race-safe onder concurrent writes (retries, dubbelkliks, mobile reconnects).
  // Fix: single-row SELECT voor MAX, insert, retry-on-unique-violation met
  // bounded attempts.
  ...
  async function writeBet(bet, userId = null, unitEur = null) {
    ...
    // Retry-loop voor bet_id race-condition. Bounded op 5 pogingen zodat
    // infinite-loop onmogelijk is als er een echte PK-conflict-bug zit.
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (UNIQUE_VIOLATION_RE.test(String(err.message || ''))) {
        // Another writer took this id; recompute and retry.
        continue;
      }
