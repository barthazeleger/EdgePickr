...
const { oddsBucket } = require('./picks');
...
module.exports = function createBetsData(deps) {
  ...
  async function writeBet(bet, userId = null, unitEur = null) {
    ...
    // v15.4: derive odds-bucket bij write-time zodat retrospectieve audits
    // (scripts/odds-bucket-audit.js) niet elk row hoeven te re-deriveren.
    // Schema-tolerant: kolom valt in fallback-tier eruit als migratie nog niet
    // gerund is (zie tier-2 hieronder).
    const oddsBucketTag = oddsBucket(Number(bet.odds));
    const basePayload = {
    ...
    if (!_sequenceKnownBroken && !bet.id) {
      ...
      if (seqResult.columnMismatch) {
        // Probeer schema-tiers met sequence (no fixture_id, dan odds_bucket,
        // dan was_preferred, dan unit_at_time). Schema-graceful zodat oude DB
        // ook nog werkt.
        ...
        if (seq2.columnMismatch) {
          // v15.4: strip odds_bucket eerst (laatste toevoeging, meest waarschijnlijk de mismatch).
          const { odds_bucket: _ob, ...noOddsBucket } = basePayload;
          const seq2b = await _trySequenceInsert(noOddsBucket, null);
          if (seq2b.ok) { bet.id = seq2b.betId; await maybeApplyBookieBalanceOnWrite(userId, balanceHookArgs); return; }
          if (seq2b.columnMismatch) {
            // v12.2.8: strip was_preferred_at_log_time (pre-migratie schema).
            const { was_preferred_at_log_time: _wp, ...noWasPref } = noOddsBucket;
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (err && isColumnError(err.message)) {
        // v15.4: strip odds_bucket (pre-migratie schema, hoogste-versie kolom eerst).
        const { odds_bucket: _ob, ...noOddsBucket } = base;
        err = await safeInsert(noOddsBucket);
        if (err && isColumnError(err.message)) {
          // v12.2.8: strip was_preferred_at_log_time (pre-migratie schema).
          const { was_preferred_at_log_time: _wp, ...noWasPref } = noOddsBucket;
    ...
    try {
      if (prevSettled && prevOutcome !== uitkomst) {
        await revertCalibration({
          ...
          odds, units, inzet, unit_at_time: row.unit_at_time, uitkomst: prevOutcome, wl: row.wl,
      ...
      if (newSettled) {
        await updateCalibration({
          datum: row.datum, wedstrijd: row.wedstrijd, markt: row.markt,
          odds, units, inzet, unit_at_time: row.unit_at_time, uitkomst, wl,
          sport: row.sport || 'football', league: row.league,
          ep: row.ep, prob: row.prob, tip: row.tip,
          was_preferred_at_log_time: row.was_preferred_at_log_time,
        }, userId);
