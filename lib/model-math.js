...
function detectMarket(markt = '') {
  ...
  // v12.2.28: F5 (baseball first 5 innings) krijgt eigen calibration-bucket.
  // Voorheen vielen F5 picks in 'over'/'under' samen met main O/U → cross-
  // contamination van baseball_over multiplier. F5 totals zijn ander product
  // (alleen 1e 5 innings, geen late-inning bullpen). Aparte bucket voorkomt
  // dat F5-resultaten Kelly-stakes voor MLB main O/U vertekenen en omgekeerd.
  if (m.includes('f5 ') || /\bf5\s/i.test(markt)) {
    if (m.includes('over')) return 'f5_over';
    if (m.includes('under')) return 'f5_under';
    // F5 ML labels (zonder over/under) krijgen f5_home/f5_away als 🏠/✈️ aanwezig
    if (m.includes('🏠') || m.includes('thuis')) return 'f5_home';
    if (m.includes('✈️') || m.includes('away')) return 'f5_away';
    return 'f5_other';
  }
...
function nflInjurySignal(homeTeamId, awayTeamId, injuryCountMap = {}, weight = 0) {
  const home = Number(injuryCountMap?.[homeTeamId]) || 0;
  const away = Number(injuryCountMap?.[awayTeamId]) || 0;
  const diff = away - home;
  const w = Number(weight);
  const safeWeight = Number.isFinite(w) ? w : 0;
  return {
    home,
    away,
    diff,
    adj: diff * 0.005 * safeWeight,
  };
}
