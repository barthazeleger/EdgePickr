...
function evaluateGraduation(rows, opts = {}) {
  ...
  for (const r of Array.isArray(rows) ? rows : []) {
    ...
    const storedCoverage = r?.playability?.preferredBookieCoverage
      || r?.playability?.preferred_bookie_coverage
      || r?.source_attribution?.oddspapi?.preferredCoverage
      || r?.source_attribution?.oddspapi?.preferred_bookie_coverage;
    if (storedCoverage && Number.isFinite(Number(storedCoverage.count))) {
      if (Number(storedCoverage.count) >= gates.min_preferred_bookie_count) slot.preferredBookieRows++;
    } else {
