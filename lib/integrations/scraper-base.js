...
async function safeFetch(url, {
  ...
  try {
    ...
    if (!r.ok) {
      return returnDetails ? { ok: false, status: r.status, error: `http_${r.status}` } : null;
    }
    if (asText) {
      const txt = await r.text();
      return returnDetails ? { ok: true, status: r.status, data: txt } : txt;
    }
    const text = await r.text();
    if (!text) return returnDetails ? { ok: false, status: r.status, error: 'empty_body' } : null;
    try {
      const data = JSON.parse(text);
      return returnDetails ? { ok: true, status: r.status, data } : data;
    } catch {
      return returnDetails ? { ok: false, status: r.status, error: 'json_parse_fail' } : null;
    }
  } catch (e) {
