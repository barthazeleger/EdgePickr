...
async function safeFetch(url, {
  ...
  if (!isUrlSafe(url, allowedHosts)) {
    return returnDetails ? { ok: false, status: 0, error: 'url_not_safe', headers: {} } : null;
  }
  if (typeof fetch !== 'function') {
    return returnDetails ? { ok: false, status: 0, error: 'no_fetch_api', headers: {} } : null;
  }
  ...
  try {
    ...
    return returnDetails ? { ok: false, status: 0, error: err.slice(0, 100), headers: {} } : null;
  } finally {
    clearTimeout(timer);
  }
}
