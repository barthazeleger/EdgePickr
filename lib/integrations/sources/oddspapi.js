...
let _warnedShapeMismatch = false;  // v15.0.10 Bug C: één-malig warn over onbekende response-shape
...
function _eventTeamNames(item = {}) {
  const teams = Array.isArray(item.teams) ? item.teams : [];
  const homeFromTeams = teams.find(t => /home/i.test(String(t?.side || t?.qualifier || '')));
  const awayFromTeams = teams.find(t => /away/i.test(String(t?.side || t?.qualifier || '')));
  const teamName = (t) => String(t?.name || t?.team || t?.title || '').slice(0, 200);
  return {
    homeTeam: String(
      item.home_team || item.homeTeam || item.home || teamName(homeFromTeams) || teams[0]?.name || ''
    ).slice(0, 200),
    awayTeam: String(
      item.away_team || item.awayTeam || item.away || teamName(awayFromTeams) || teams[1]?.name || ''
    ).slice(0, 200),
  };
}

function _eventCommenceTime(item = {}) {
  return item.commence_time || item.commenceTime || item.start_time || item.startTime || item.date || null;
}
...
async function fetchOdds(sportKey, options = {}) {
  ...
  // Pattern C: event met `markets` of `prices` array direct (zonder bookmaker-laag)
  for (const item of arr) {
    ...
    if (Array.isArray(item.bookmakers)) {
      ...
      const commenceTime = _eventCommenceTime(item);
      const { homeTeam, awayTeam } = _eventTeamNames(item);
      ...
      out.push({
        ...
        commenceTime: _eventCommenceTime(item),
        ..._eventTeamNames(item),
    ...
    } else if (Array.isArray(item.markets) || Array.isArray(item.prices) || Array.isArray(item.odds)) {
      // v15.0.10 Bug C Pattern C: event met markets/prices/odds array direct
      // (zonder bookmaker-laag). Mogelijke shape: { markets: [{bookie, market, side, price}, ...] }
      // of { prices: [{book, market, line, decimal}, ...] }.
      const eventId = String(item.id || item.fixture_id || item.event_id || '').slice(0, 60);
      const commenceTime = _eventCommenceTime(item);
      const { homeTeam, awayTeam } = _eventTeamNames(item);
      const rows = Array.isArray(item.markets) ? item.markets
                 : Array.isArray(item.prices)  ? item.prices
                 : item.odds;
      for (const row of rows) {
        if (!row || typeof row !== 'object') continue;
        const price = parseFloat(row.price || row.odds || row.decimal);
        if (!Number.isFinite(price) || price <= 1.0 || price > 2000) continue;
        const bookie = _normalizeBookieKey(row.bookmaker || row.book || row.bookie || row.book_key);
        if (!bookie) continue;
        out.push({
          source: 'oddspapi',
          eventId, commenceTime, homeTeam, awayTeam,
          bookie,
          market: _mapMarketKey(row.market || row.market_key || row.type, sport),
          marketRaw: String(row.market || row.market_key || row.type || '').slice(0, 30),
          selection: String(row.name || row.outcome || row.selection || row.side || '').slice(0, 200),
          line: row.point != null ? parseFloat(row.point) : (row.line != null ? parseFloat(row.line) : null),
          price,
          lastUpdate: row.last_update || row.updated_at || item.last_update || null,
        });
      }
    }
  }
  // v15.0.10 Bug C diagnostic: log first item-shape if response had items but
  // we extracted 0 quotes. One-shot per process. Helps identify unknown shapes
  // on next live scan zonder permanent log-spam.
  if (arr.length > 0 && out.length === 0 && !_warnedShapeMismatch) {
    _warnedShapeMismatch = true;
    try {
      const sample = arr[0];
      const keys = sample && typeof sample === 'object' ? Object.keys(sample).slice(0, 15) : [];
      const sampleStr = JSON.stringify(sample, null, 0).slice(0, 800);
      console.warn(`[oddspapi] fetchOdds: ${arr.length} items in response but parser extracted 0 quotes. Sample-keys=[${keys.join(',')}] sample=${sampleStr}`);
    } catch (_) { /* swallow */ }
  }
...
module.exports = {
  ...
  _eventTeamNames,
  _eventCommenceTime,
  _resetUsage: () => { _quotaUsed = 0; _quotaRemaining = MONTHLY_QUOTA; _quotaUpdatedAt = null; _warnedNoSport = false; _warnedShapeMismatch = false; },
};
