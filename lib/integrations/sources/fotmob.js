...
// v10.9.2: FotMob frontend Referer. FotMob valideert een signed header
// "x-mas" die per-dag hash is. Zonder dat werkt de API maar toont mogelijk
// beperkte data. We doen best-effort met Referer; als het niet werkt vangen
// breaker + aggregator-fallback dat af.
const EXTRA_HEADERS = {
...
async function healthCheck() {
  ...
  // v12.5.11: returnDetails:true voor diagnose-info (status + error).
  const details = await safeFetch(url, {
    allowedHosts: ALLOWED, extraHeaders: EXTRA_HEADERS, returnDetails: true,
  });
  const latency = Date.now() - t0;
  const healthy = details && details.ok && details.data && (Array.isArray(details.data.suggestions) || Array.isArray(details.data));
