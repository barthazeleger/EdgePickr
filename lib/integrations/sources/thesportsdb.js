...
// Backwards-compat alias voor bestaande code-paden + tests.
...
async function fetchStandings(leagueId, season) {
  ...
  const cacheKey = `tsdb:standings:${leagueId}:${seasonStr}`;
  const cached = _caches.medium.get(cacheKey);
  if (cached !== undefined) return cached;

  const qs = seasonStr ? `?l=${encodeURIComponent(leagueId)}&s=${encodeURIComponent(seasonStr)}`
...
async function fetchEventLineup(eventId) {
  if (!eventId) return [];
  const cacheKey = `tsdb:lineup:${eventId}`;
  const cached = _caches.short.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookuplineup.php?id=${encodeURIComponent(eventId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
...
async function fetchEventStats(eventId) {
  if (!eventId) return [];
  const cacheKey = `tsdb:eventstats:${eventId}`;
  const cached = _caches.short.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookupeventstats.php?id=${encodeURIComponent(eventId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
...
async function fetchEventTimeline(eventId, isFinal = false) {
  if (!eventId) return [];
  const cacheKey = `tsdb:timeline:${eventId}:${isFinal ? 'final' : 'live'}`;
  const c = isFinal ? _caches.week : _caches.short;
  const cached = c.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookuptimeline.php?id=${encodeURIComponent(eventId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
...
async function fetchEventTV(eventId) {
  if (!eventId) return [];
  const cacheKey = `tsdb:tv:${eventId}`;
  const cached = _caches.day.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookuptv.php?id=${encodeURIComponent(eventId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
...
async function _fetchLeagueEvents(leagueId, direction) {
  ...
  const cacheKey = `tsdb:league-${direction}:${leagueId}`;
  const cached = _caches.hour.get(cacheKey);
  if (cached !== undefined) return cached;

  const file = direction === 'next' ? 'eventsnextleague.php' : 'eventspastleague.php';
  const url = `${BASE_V1}/${file}?id=${encodeURIComponent(leagueId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
  const raw = data?.events || [];
  if (!Array.isArray(raw)) { _caches.hour.set(cacheKey, []); return []; }
...
async function fetchVenue(venueId) {
  ...
  const cacheKey = `tsdb:venue:${venueId}`;
  const cached = _caches.week.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookupvenue.php?id=${encodeURIComponent(venueId)}`;
  const { data, called } = await _get(url);
  if (!called) return null;
  ...
  const venue = {
    source: 'thesportsdb',
    venueId: raw.idVenue ? String(raw.idVenue) : String(venueId),
...
async function fetchTeamRoster(teamId) {
  if (!teamId) return [];
  const cacheKey = `tsdb:roster:${teamId}`;
  const cached = _caches.medium.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V1}/lookup_all_players.php?id=${encodeURIComponent(teamId)}`;
  const { data, called } = await _get(url);
  if (!called) return [];
...
async function fetchTeamFullSchedule(teamId) {
  if (!teamId) return [];
  const cacheKey = `tsdb:full-schedule:${teamId}`;
  const cached = _caches.medium.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V2}/schedule/full/team/${encodeURIComponent(teamId)}`;
  const { data, called } = await _get(url, /*useV2=*/ true);
  if (!called) return [];
...
async function fetchScheduleByVenue(venueId, direction = 'next') {
  ...
  const cacheKey = `tsdb:venue-schedule-${direction}:${venueId}`;
  const cached = _caches.hour.get(cacheKey);
  if (cached !== undefined) return cached;

  const url = `${BASE_V2}/schedule/${direction}/venue/${encodeURIComponent(venueId)}`;
  const { data, called } = await _get(url, /*useV2=*/ true);
  if (!called) return [];
  const raw = data?.schedule || data?.events || [];
  if (!Array.isArray(raw)) { _caches.hour.set(cacheKey, []); return []; }
