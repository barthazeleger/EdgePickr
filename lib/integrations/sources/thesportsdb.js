...
//
// v15.0.2: per-endpoint breakdown. Operator wil weten waar de daily calls
// heen gaan zodat under-utilization (35/scan vs ~30k/dag budget) per
// endpoint diagnostisch wordt. byEndpoint key is path-tail zonder .php
// (v1) of het type-segment (v2 — bv. livescore/schedule). _resetUsage
// reset beide.
let _callsToday = 0;
let _callsTodayDate = null;
let _callsByEndpoint = Object.create(null);
function _amsterdamDate() {
  return new Date().toLocaleDateString('sv-SE', { timeZone: 'Europe/Amsterdam' });
}
function _endpointKeyFromUrl(url) {
  if (!url || typeof url !== 'string') return 'unknown';
  try {
    const parsed = new URL(url);
    const segs = parsed.pathname.split('/').filter(Boolean);
    if (parsed.pathname.includes('/api/v2/')) {
      // V2: /api/v2/json/<type>/...   — pak het type-segment (livescore/schedule).
      const idx = segs.indexOf('json');
      const type = idx >= 0 && segs[idx + 1] ? segs[idx + 1] : segs[segs.length - 1];
      return String(type || 'unknown').toLowerCase().slice(0, 40) || 'unknown';
    }
    // V1: /api/v1/json/<key>/<file.php> — pak file zonder .php.
    const tail = segs[segs.length - 1] || 'unknown';
    return String(tail).replace(/\.php$/i, '').toLowerCase().slice(0, 40) || 'unknown';
  } catch (_) {
    return 'unknown';
  }
}
function _bumpUsageCounter(url) {
  const d = _amsterdamDate();
  if (_callsTodayDate !== d) {
    _callsTodayDate = d;
    _callsToday = 0;
    _callsByEndpoint = Object.create(null);
  }
  _callsToday++;
  const key = _endpointKeyFromUrl(url);
  _callsByEndpoint[key] = (_callsByEndpoint[key] || 0) + 1;
}
...
// v15.0.2: byEndpoint breakdown toegevoegd voor under-utilization diagnose.
// v15.0.12: theoretical daily budget + per-endpoint percentages zodat
// /api/admin/v2/tsdb-utilization kan tonen welke endpoints "hot" zijn t.o.v.
// het rate-limit. Premium 100/min ≈ 144000/dag, free key ≈ 57600/dag.
function getUsage() {
  const dailyBudget = Math.floor((24 * 60 * 60 * 1000) / RATE_LIMIT_MS);
  const byEndpointPercent = Object.create(null);
  for (const [ep, n] of Object.entries(_callsByEndpoint)) {
    byEndpointPercent[ep] = +(((n / dailyBudget) * 100)).toFixed(3);
  }
  return {
    source: SOURCE_NAME,
    callsToday: _callsToday,
    byEndpoint: { ..._callsByEndpoint },
    byEndpointPercent,
    dailyBudget,
    utilizationPct: +(((_callsToday / dailyBudget) * 100)).toFixed(3),
...
// v15.3.0: Event-detail per event-ID. v1 lookupevent.php returnt {events:[{...}]}
// met final-score velden zodra status FT/AET/PEN is. Gebruikt voor expansion-
// shadow settlement (paper-trading) waar fixture_id = TSDB idEvent (string).
// Returnt null bij niet-finished/onbekend event of als source disabled.
async function fetchEventDetail(eventId) {
  const id = _validId(eventId);
  if (!id) return null;
  const cacheKey = `tsdb:event:${id}`;
  // Short cache (30min) voor live events; week-cache lifetime na FT.
  const cachedShort = _caches.short.get(cacheKey);
  if (cachedShort !== undefined) return cachedShort;
  const cachedWeek = _caches.week.get(cacheKey);
  if (cachedWeek !== undefined) return cachedWeek;

  const url = `${BASE_V1}/lookupevent.php?id=${encodeURIComponent(id)}`;
  const { data, called } = await _get(url);
  if (!called) return null;
  const raw = (data?.events || data?.event || [])[0];
  if (!raw) {
    // API answered "no match" → cache as null negative voor 30min, niet week.
    _caches.short.set(cacheKey, null);
    return null;
  }

  const status = String(raw.strStatus || raw.strProgress || '').toLowerCase();
  const isFinished = /^(ft|fte|aet|pen|finished|match\s*finished)$/i.test(status);
  const homeScore = parseInt(raw.intHomeScore, 10);
  const awayScore = parseInt(raw.intAwayScore, 10);

  const event = {
    source: 'thesportsdb',
    eventId: raw.idEvent ? String(raw.idEvent) : id,
    leagueId: raw.idLeague ? String(raw.idLeague) : null,
    leagueName: String(raw.strLeague || '').slice(0, 120),
    homeTeam: String(raw.strHomeTeam || '').slice(0, 200),
    awayTeam: String(raw.strAwayTeam || '').slice(0, 200),
  ...
  // Finished events zijn immutable → week cache. Live/scheduled → short cache
  // zodat we elke 30min refreshen tot ze finished zijn.
  if (isFinished && Number.isFinite(event.homeScore) && Number.isFinite(event.awayScore)) {
    _caches.week.set(cacheKey, event);
  } else {
    _caches.short.set(cacheKey, event);
  }
  return event;
}
...
module.exports = {
  ...
  // v15.3.0: event-detail per ID voor TSDB-backed expansion-shadow settlement.
  fetchEventDetail,
