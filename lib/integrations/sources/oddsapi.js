...
async function _get(path, params = {}, options = {}) {
  ...
  breaker.onSuccess();
  // Quota-headers parsen — authoritative source voor remaining/used.
  if (details.headers) {
    const remaining = details.headers['x-requests-remaining'];
    const used = details.headers['x-requests-used'];
    if (remaining != null && Number.isFinite(parseInt(remaining, 10))) {
      _quotaRemaining = parseInt(remaining, 10);
    }
    if (used != null && Number.isFinite(parseInt(used, 10))) {
      _quotaUsed = parseInt(used, 10);
    }
    _quotaUpdatedAt = new Date().toISOString();
  } else if (quotaCost) {
    // Fallback: lokaal incrementeren als headers niet beschikbaar.
    _quotaUsed++;
...
module.exports = {
  ...
  _normalizeBookieKey,
  _mapMarketKey,
  _resetUsage: () => { _quotaUsed = 0; _quotaRemaining = MONTHLY_QUOTA; _quotaUpdatedAt = null; },
};
