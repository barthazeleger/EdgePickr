...
// v15.0.12: Team roster per teamName. Resolvet teamId via TSDB findTeamId
// (day-cached) en fetcht roster via fetchTeamRoster (medium 6h cached).
// Hergebruikt voor injury-cross-check signal. Returnt [] als source disabled.
async function getTeamRoster(sport, teamName) {
  if (!teamName) return [];
  const reg = SPORT_SOURCES[sport];
  if (!reg) return [];
  // TSDB is de enige source voor roster vandaag (sofascore disabled).
  const tsdbSrc = (reg.h2h || []).find(s => s && typeof s.findTeamId === 'function' && typeof s.fetchTeamRoster === 'function');
  if (!tsdbSrc) return [];
  try {
    const team = await tsdbSrc.findTeamId(teamName, sport);
    if (!team || !team.id) return [];
    return await tsdbSrc.fetchTeamRoster(team.id);
  } catch {
    return [];
  }
}
...
// v15.0.2: League scoring baseline. Pakt de laatste-15 fixtures van een
// liga via TSDB `fetchLeaguePast`, en past Bayesian-shrunk gemiddelde-goals
// toe als additieve OU-prior signal. Day-cached door TSDB-source.
//
// Caller-side behavior: gebruikt alleen voor sports die TSDB als
// schedule-source hebben (football/basketball/etc.). Returnt null als
// sample te thin is of source disabled.
async function getLeagueScoringBaseline(sport, leagueId, opts = {}) {
  if (!leagueId) return null;
  const reg = SPORT_SOURCES[sport];
  // Hergebruik de schedule-bron-keten — TSDB staat als eerste in de meeste sports.
  if (!reg || !Array.isArray(reg.schedule) || !reg.schedule.length) return null;
  const tsdbSrc = reg.schedule.find(s => s && typeof s.fetchLeaguePast === 'function');
  if (!tsdbSrc) return null;
  let events = [];
  try {
    events = await tsdbSrc.fetchLeaguePast(leagueId);
  } catch { return null; }
  if (!Array.isArray(events) || events.length === 0) return null;
  const { computeLeagueBaseline } = require('../signals/league-scoring-baseline');
  return computeLeagueBaseline(events, opts);
}
