...
// v12.7.0-pre3 (v13.0 Phase 3): OddsAPI free-tier voor odds + events fallback.
...
// Dedup voor odds-quotes op (eventId, bookie, market, line, selection).
// Behoudt de eerste; als zelfde key uit verschillende sources komt met
// verschillende prijs, returnt die als source-disagreement in `_anomalies`
// zodat callers (server.js) een bookie_anomaly inbox-warning kunnen sturen.
function _dedupOdds(quotes, anomalyThresholdPct = 5) {
  const seen = new Map(); // key → {quote, sources: [{source, price}]}
  const anomalies = [];
  for (const q of quotes) {
    if (!q || !q.eventId || !q.bookie || !q.market) continue;
    const line = q.line == null ? '' : String(q.line);
    const key = `${q.eventId}::${q.bookie}::${q.market}::${line}::${q.selection || ''}`;
    if (!seen.has(key)) {
    ...
    // Detecteer prijs-disagreement >threshold tussen sources voor dezelfde
    // (bookie, market, line, selection) combo. Onderscheid is een echt signaal:
    // bookmaker offer-tijd verschilde, of één van de sources is stale.
    const minPrice = Math.min(...entry.sources.map(s => s.price));
    const maxPrice = Math.max(...entry.sources.map(s => s.price));
    if (minPrice > 1 && (maxPrice - minPrice) / minPrice * 100 > anomalyThresholdPct) {
      anomalies.push({
        eventId: q.eventId, bookie: q.bookie, market: q.market, line: q.line,
        selection: q.selection, sources: entry.sources, deltaPct: +(((maxPrice - minPrice) / minPrice) * 100).toFixed(2),
      });
    }
  }
  return {
      ...
      if (src === oddsapi && typeof src.resolveOddsApiKey === 'function') {
        ...
        if (sportKey && typeof src.fetchEvents === 'function') {
          ...
          if (Array.isArray(events) && events.length) {
            // Filter op date als gevraagd (events bevatten commenceTime ISO).
            const filtered = events.filter(ev => {
              if (!ev.commenceTime) return false;
              return ev.commenceTime.slice(0, 10) === date;
            });
