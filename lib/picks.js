...
const { kellyToUnits, getKellyFraction, detectMarket } = require('./model-math');

// v15.4 · Variance-bom-gate constants. Per-odds-bucket MIN_EP bump dempt de
// low-information edge die hoge-odds picks structureel claimen (long-tail
// variance i.p.v. echte +CLV). Boundaries komen uit PLAN §5.1.
//   low  · odd ≤ 2.0 → MIN_EP ongewijzigd
//   mid  · 2.0 < odd ≤ 3.0 → MIN_EP +1pp
//   high · odd > 3.0       → MIN_EP +2pp · plus 0.5U kelly-cap behalve bij
//                                            ep·(odd-1) > exception threshold
const ODDS_BUCKET_MIN_EP_BUMP = Object.freeze({ low: 0.00, mid: 0.01, high: 0.02 });
const HIGH_ODDS_KELLY_CAP_UNITS = '0.5U';
// Cap-uitzondering: alleen écht overtuigend hoge expected gross-profit
// laat de high-odds-pick aan zijn volledige Kelly-stake. ep·(odd-1) > 1.50
// betekent ~ "verwachte bruto-winst > 1,5× inzet" — typisch alleen bij
// odd ≈ 3.5 met ep > 0.60 of odd ≈ 4 met ep > 0.50, dus zeldzame outliers.
// Doctrine-zuiver: zonder bewezen edge cap-default-aan, escape alleen
// wanneer model materiaal sterker zit dan markt.
const HIGH_ODDS_EXCEPTION_THRESHOLD = 1.50;
...
function unitsToFloat(u) {
  if (typeof u !== 'string') return 0;
  const m = /^([0-9]*\.?[0-9]+)U/.exec(u);
...
function createPickContext(options = {}) {
  return {
    ...
    resolveMinEp: typeof options.resolveMinEp === 'function' ? options.resolveMinEp : null,
    // v12.5.1: hot-toggle voor de v12.5.0 conviction-route doctrine. Als
    // de wekelijkse `evaluateConvictionDoctrine` (lib/conviction-doctrine.js)
    // beslist tot 'revert' op basis van shadow-CLV-evidence, zet caller
    // `convictionDisabled=true`. mkP's adaptive ep-gap valt dan terug naar
    // het v12.4.x 0.03-gedrag voor sigCount ≥ 6 — hot-rollback zonder code-
    // deploy.
    convictionDisabled: !!options.convictionDisabled,
    // v15.4 · varianceGateCounter is een mutable telemetry-sink die mkP
    // invult bij block (per-bucket MIN_EP-bump faalt) en cap (high-odds
    // kelly-cap kicks in). Caller geeft één gedeeld object door aan alle
    // sport-pick-factories zodat scanTelemetry direct kan rolluppen.
    // Shape: { blocks: { low, mid, high }, capped: { low, mid, high } }.
    varianceGateCounter: options.varianceGateCounter && typeof options.varianceGateCounter === 'object'
      ? options.varianceGateCounter
      : null,
  };
}
...
function buildPickFactory(MIN_ODDS = 1.60, calibEpBuckets = {}, options = {}) {
  ...
  const { drawdownMultiplier, activeUnitEur, adaptiveMinEdge, sport, executionMetrics: ctxMetrics, resolveExecutionMetrics, onCandidate, resolveMinEp, convictionDisabled, varianceGateCounter } = ctx;
  const bumpVarianceGate = (kind, bucket) => {
    if (!varianceGateCounter || !bucket) return;
    if (!varianceGateCounter[kind] || typeof varianceGateCounter[kind] !== 'object') {
      varianceGateCounter[kind] = { low: 0, mid: 0, high: 0 };
    }
    varianceGateCounter[kind][bucket] = (varianceGateCounter[kind][bucket] || 0) + 1;
  };
  ...
  // v12.5.3: per-reason sigCount-distributie. Wordt parallel aan dropReasons
  // bijgehouden zodat `formatDropReasons` straks per drop-reason óók de
  // gemiddelde sigCount + tellers per bucket toont. Diagnose-instrument om
  // te zien of de v12.5.0 doctrine-pivot (sigCount-afhankelijke ep-gap) de
  // juiste populatie raakt: een ep_too_close-drop met sigCount≥6 is een
  // pick die net niet door de loosened gate kwam; sigCount<3 is het soort
  // dat we sowieso niet zouden willen.
  const dropSigCounts = Object.create(null);
  const bumpDrop = (reason, sigCount) => {
    dropReasons[reason] = (dropReasons[reason] || 0) + 1;
    if (Number.isFinite(sigCount) && sigCount >= 0) {
      if (!dropSigCounts[reason]) dropSigCounts[reason] = [];
      dropSigCounts[reason].push(sigCount);
    }
  };
  ...
  // v12.4.1: ook async-rejections vangen. v12.4.0 swallowt'e alleen sync
  // throws; een async onCandidate die rejects ontsnapte naar
  // unhandledRejection (Node 22 default = process exit). Promise.resolve
  // wraps zowel sync return-values als al-promise return-values.
  const emitCandidate = (pushed, dropReason, fixtureMeta, label, pick) => {
    if (!onCandidate) return;
    try {
      Promise.resolve(onCandidate({ pushed, dropReason: dropReason || null, fixtureMeta: fixtureMeta || null, label, sport, pick: pick || null }))
        .catch(() => { /* swallow async rejection */ });
    } catch (_) { /* swallow sync throw */ }
  };
  ...
  const mkP = (match, league, label, odd, reason, prob, boost=0, kickoff=null, bookie=null, signals=null, referee=null, fixtureMeta=null) => {
    // v12.5.3: sigCount vroeg berekenen zodat álle drop-paden hem kunnen
    // meegeven aan bumpDrop voor distributie-telemetrie (zie formatDropReasons).
    ...
    const dropPick = (rsn) => { bumpDrop(rsn, sigCount); emitCandidate(false, rsn, fixtureMeta, label); };
    if (!odd || odd < 1.10) { dropPick('price_too_low'); return; }
    const ip = 1/odd;
    const ep = Math.min(0.88, ip + boost);
    const marketType = fixtureMeta?.marketType || detectMarket(label);
    let minEp = MIN_EP;
    if (resolveMinEp) {
      try {
        const resolved = resolveMinEp({ sport, marketType, label, fixtureMeta });
        if (Number.isFinite(resolved) && resolved > 0 && resolved < 1) minEp = resolved;
      } catch (_) { minEp = MIN_EP; }
    }
    // v15.4 variance-bom-gate: per-odds-bucket MIN_EP bump bovenop resolveMinEp.
    // Hogere odds-buckets vragen meer ep-marge omdat long-tail variance daar
    // structureel als "edge" verschijnt zonder echte +CLV. Plan §5.1 + Codex #2.
    const bucket = oddsBucket(odd);
    const bucketBump = bucket ? (ODDS_BUCKET_MIN_EP_BUMP[bucket] || 0) : 0;
    const bucketMinEp = minEp + bucketBump;
    if (ep < bucketMinEp) {
      // Block-tellers gaan naar de varianceGate-sink. Bij low-bucket is bump=0
      // dus deze drop is functioneel identiek aan een ep_below_min drop, maar
      // we tellen 'm óók in blocks.low zodat operator per scan-log kan zien
      // welke bucket hoeveel pre-pick drops produceerde.
      bumpVarianceGate('blocks', bucket || 'low');
      dropPick('ep_below_min'); return;
    }
    // v12.5.0: confidence-gewogen ep-gap. Doctrine "markt = baseline truth"
    // blijft — model moet nog steeds materieel boven markt-implied zitten,
    // maar HOEVEEL erboven hangt af van signaal-bewijs achter de boost.
    ...
    // v12.5.1: convictionDisabled toggle — wekelijkse doctrine-evaluator kan
    // de high-conviction loosening (0.02) terugdraaien op slecht CLV-bewijs.
    // Dan: alle sigCount-buckets ≥ 3 vallen samen op 0.03 (= v12.4.x).
    const epGap = convictionDisabled
      ? (sigCount >= 3 ? 0.03 : 0.04)
      : (sigCount >= 6 ? 0.02 : sigCount >= 3 ? 0.03 : 0.04);
    if (ep <= ip + epGap) { dropPick('ep_too_close_to_market'); return; }
    ...
    // v12.5.0: conviction_route flag. true betekent: deze pick zou onder de
    // oude 3pp-gate gedropt zijn, en kwam alleen door dataConf=1.0 (sigCount
    // ≥ 6) heen. Wordt na settlement gebruikt om CLV-/ROI-evidence te
    // splitten tussen edge-track en conviction-track. Bij sigCount in [3,5]
    // is de gate ongewijzigd 0.03 → conviction_route=false.
    const convictionRoute = sigCount >= 6 && epGap === 0.02 && (ep - ip) < 0.03;
    const audit = {
      ...
      conviction_route: convictionRoute,
    };
    ...
    let u  = kellyToUnits(hk);
    const edge = Math.round((ep * odd - 1) * 100 * 10) / 10;

    // v15.4 variance-bom-gate: high-odds-cap. Picks met odd > 3.0 krijgen max
    // 0.5U tenzij ep·(odd-1) > drempel — de uitzondering vangt de zeldzame
    // gevallen waar het verwachte gross-profit per unit ruim genoeg is om
    // legitiem buiten variance-territory te zitten.
    if (bucket === 'high' && (ep * (odd - 1)) <= HIGH_ODDS_EXCEPTION_THRESHOLD) {
      const capUnits = unitsToFloat(HIGH_ODDS_KELLY_CAP_UNITS);
      if (unitsToFloat(u) > capUnits) {
        u = HIGH_ODDS_KELLY_CAP_UNITS;
        bumpVarianceGate('capped', 'high');
      }
    }

    const uNum = parseFloat(u);
    // v12.5.0: dataConf^1.5 ranking-lift in expectedEur. server.js sorteert
    // top-5 op expectedEur (server.js:6871/6898) — een pure dataConf^1
    // multiplier onderschat 6+ signaal-picks bij gelijke edge. ^1.5 geeft
    // sigCount=6 (dataConf=1.0) ongewijzigd 1×, sigCount=3 (0.70) → 0.586×
    // i.p.v. 0.70× t.o.v. dataConf=1, sigCount=1-2 (0.55) → 0.408× i.p.v.
    // 0.55×. Resultaat: high-conviction picks staan vaker top-5 zonder dat
    // kelly/units (= stake-sizing) wordt aangeraakt — die blijven edge-driven.
    const dataConfRank = Math.pow(dataConf, 1.5);
    const expectedEur = +(uNum * activeUnitEur * (edge / 100) * dataConfRank).toFixed(2);
  ...
  return { picks, combiPool, mkP, dropReasons, dropSigCounts };
}
 ...
 *
 * v12.5.3: optionele 2e arg `dropSigCounts` (object met arrays per reason)
 * verrijkt de output met sigCount-distributie. Diagnose: bij `ep_too_close
 * _to_market=3 (sigCount avg=4.7, ≥6=1)` weet operator dat 1 van de 3 met
 * de v12.5.0 doctrine-pivot net door zou kunnen komen — concrete signal
 * voor verdere tuning. Backwards-compat: zonder 2e arg → oude format.
 */
function formatDropReasons(dropReasons, dropSigCounts) {
  if (!dropReasons || typeof dropReasons !== 'object') return null;
  const entries = Object.entries(dropReasons)
    .filter(([, n]) => n > 0)
    .sort((a, b) => b[1] - a[1]);
  if (!entries.length) return null;
  const enrich = dropSigCounts && typeof dropSigCounts === 'object';
  return entries.map(([k, n]) => {
    if (!enrich) return `${k}=${n}`;
    const arr = Array.isArray(dropSigCounts[k]) ? dropSigCounts[k] : null;
    if (!arr || arr.length === 0) return `${k}=${n}`;
    const avg = arr.reduce((s, v) => s + v, 0) / arr.length;
    const highConv = arr.filter(v => v >= 6).length;
    return `${k}=${n} (sigCount avg=${avg.toFixed(1)}, ≥6=${highConv})`;
  }).join(' · ');
}
...
// toward a sport-wide prior so 2/2 or 3/3 meetings do not overdrive picks.
// v14.0 Phase C: prior + K nu data-tunable per sport via calib.json. Defaults
// match pre-migratie hardcoded waardes voor backwards-compat.
const BTTS_H2H_PRIOR = 0.52;
const BTTS_H2H_PRIOR_K = 8;
function calcBTTSProb({ h2hBTTS, h2hN, hmAvgGF, awAvgGF, prior = BTTS_H2H_PRIOR, priorK = BTTS_H2H_PRIOR_K }) {
  const n = h2hN || 0;
  const btts = h2hBTTS || 0;
  const usedPrior = Number.isFinite(prior) && prior > 0 && prior < 1 ? prior : BTTS_H2H_PRIOR;
  const usedK     = Number.isFinite(priorK) && priorK > 0 ? priorK : BTTS_H2H_PRIOR_K;
  const h2hRate = (btts + usedPrior * usedK) / (n + usedK);
...
module.exports = {
  ...
  oddsBucket,
  ODDS_BUCKET_MIN_EP_BUMP,
  HIGH_ODDS_KELLY_CAP_UNITS,
  HIGH_ODDS_EXCEPTION_THRESHOLD,
};
