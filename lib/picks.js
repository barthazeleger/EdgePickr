...
function buildPickFactory(MIN_ODDS = 1.60, calibEpBuckets = {}, options = {}) {
  ...
  const mkP = (match, league, label, odd, reason, prob, boost=0, kickoff=null, bookie=null, signals=null, referee=null, fixtureMeta=null) => {
    if (!odd || odd < 1.10) { bumpDrop('price_too_low'); return; }
    const ip = 1/odd;
    const ep = Math.min(0.88, ip + boost);
    if (ep < MIN_EP) { bumpDrop('ep_below_min'); return; }
    if (ep <= ip + 0.03) { bumpDrop('ep_too_close_to_market'); return; }
    const k = ((ep*(odd-1)) - (1-ep)) / (odd-1);
    if (k <= 0.015) { bumpDrop('kelly_too_low'); return; }
    ...
    if (sigCount === 0) { bumpDrop('no_signals'); return; }
    ...
    // 1 valse edge" weegt zwaarder dan dampen wanneer model én markt > 25pp
    // uit elkaar staan zonder signal-attribution. In dat geval is óf het
    // model verkeerd óf de markt heeft info die wij missen — beide cases
    // willen we niet betten. Onder 25pp blijft dampen het juiste antwoord.
    if (auditSuspicious && (Math.abs(probGap) > 25 || Math.abs(baseGap) > 25)) {
      bumpDrop('extreme_divergence');
      return;
    }
    ...
    if (metrics) {
      ...
      if (gated.skip === true) { bumpDrop('execution_gate_skip'); return; } // hard skip — geen pick
    }
    ...
    if (adaptiveMinEdge) {
      ...
      if (edge < requiredEdgePct) { bumpDrop('edge_below_adaptive'); return; }
    }
