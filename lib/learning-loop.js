...
module.exports = function createLearningLoop(deps) {
  ...
  async function updateCalibration(bet, userId = null) {
    ...
    if (mk.n >= 20) {
      ...
      const rawDelta = Math.max(-0.30, Math.min(0.30, profitPerBet * 0.03));
      // v12.5.4: confidence-ramp. Bij n=20 heeft een 1.90-odds-markt nog ±18pp
      // CI op de winrate — vol-delta multiplier is variance, niet edge. Scale
      // proportioneel: n=20 → 0× delta (multiplier blijft 1.00), n=60 → 0.5×,
      // n=100 → 1× volle delta. Sluit aan bij operator-doctrine "stake-ups
      // vragen bewijs, niet enthousiasme".
      const confidence = Math.max(0, Math.min(1, (mk.n - 20) / 80));
