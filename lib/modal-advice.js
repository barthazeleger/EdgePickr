...
function computeModalAdvice(input) {
  ...
  // Tier 5: hogere odds → units mogen stijgen, maar beperkt.
  // v12.1.0 (operator-rapport): voorheen capte dit op `origUnits` waardoor
  // een +5% betere odd NOOIT meer stake gaf ("0.5U blijft 0.5U"). Tegelijk
  // sprong de score wél naar 10/10 (max(origScore, freshScore)), wat
  // inconsistent was. Nu: units mogen één bucket omhoog bij ≥4% betere odds
  // (≥2% 1 bucket cap), en score volgt de aangepaste units.
  if (diffPct > 2) {
    let rec;
    if (!origUnits) {
      rec = pureRec;
    } else {
      const origIdx = UNIT_BUCKETS.indexOf(origUnits);
      // +2-4% → blijf op origUnits. +4% of meer → 1 bucket omhoog (gecapt op
      // pureRec zodat je nooit boven pure Kelly uitkomt).
      if (diffPct >= 4 && origIdx >= 0 && origIdx < UNIT_BUCKETS.length - 1) {
        const oneUp = UNIT_BUCKETS[origIdx + 1];
        rec = Math.min(oneUp, pureRec);
      } else {
        rec = Math.min(pureRec, origUnits);
      }
    }
    // Score consistent met rec-units: niet meer max(origScore, freshScore)
    // die afwijkt van rec. Schat score uit hk-equivalent van rec.
    const scoreForRec = rec > origUnits ? freshScore : effOrigScore;
    return {
      severity: 'better', recUnits: rec,
      diff, diffPct, edge, dampedEdge, kelly, hk, dampingFactor,
      scoreVal: scoreForRec,
