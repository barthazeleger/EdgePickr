...
async function writePickCandidate(supabase, args) {
  ...
  try {
    const row = {
      ...
      source_attribution:  args.sourceAttribution && typeof args.sourceAttribution === 'object' ? args.sourceAttribution : {},
      sharp_anchor:         args.sharpAnchor && typeof args.sharpAnchor === 'object' ? args.sharpAnchor : {},
      playability:          args.playability && typeof args.playability === 'object' ? args.playability : {},
    };
    ...
    // v12.5.0: conviction_route flag voor CLV-monitoring per pick-track.
    if (args.convictionRoute != null) row.conviction_route = !!args.convictionRoute;
...
function candidatePayloadExtras(args = {}) {
  const debug = args.debug || {};
  const sport = args.sport || debug.sport || null;
  const sourceAttribution = args.sourceAttribution && typeof args.sourceAttribution === 'object'
    ? args.sourceAttribution
    : (debug.sourceAttribution && typeof debug.sourceAttribution === 'object')
      ? debug.sourceAttribution
      : (sport ? { apiSports: { primary: true } } : {});
  return {
    sourceAttribution,
    sharpAnchor: args.sharpAnchor || debug.sharpAnchor || {},
    playability: args.playability || debug.playability || {},
  };
}
...
async function recordMl2WayEvaluation(args) {
  ...
  try {
    ...
    // v12.5.0: convictionSelections is een Set<selectionKey> (bv. {'home','away'})
    // van picks die door de loosened ep-gate zijn gekomen. Caller bouwt 'm op
    // via de mkP onCandidate-hook (pick.audit.conviction_route).
    const convictionSet = (args.convictionSelections instanceof Set) ? args.convictionSelections : null;
    const evaluate = (side, edge, fairProb, best) => {
      ...
      writePickCandidate(supabase, {
        ...
        convictionRoute: convictionSet ? convictionSet.has(side) : false,
        ...candidatePayloadExtras(args),
      }).catch(() => {});
    };
...
async function recordTotalsEvaluation(args) {
  ...
  try {
    ...
    if (!runId) return;
    // v12.5.0: zie recordMl2WayEvaluation voor convictionSelections-rationale.
    const convictionSet = (args.convictionSelections instanceof Set) ? args.convictionSelections : null;
    const evaluate = (side, edge, fairProb, best) => {
      let rejected = null;
  ...
  try {
    ...
    const evaluate = (sel, edge, fairProb, best) => {
      ...
      writePickCandidate(supabase, {
        ...
        passedFilters: !rejected, rejectedReason: rejected,
        signals: matchSignals,
        convictionRoute: convictionSet ? convictionSet.has(sel) : false,
        ...candidatePayloadExtras(args),
      }).catch(() => {});
    };
